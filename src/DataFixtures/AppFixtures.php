...
class AppFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 0; $i < 35; $i++) {
            $title = $faker->unique()->sentence(3); // Générer un titre en français

            // Vérifier si le deal existe déjà par titre
            $existingDeal = $manager->getRepository(Deal::class)->findOneBy(['title' => $title]);
